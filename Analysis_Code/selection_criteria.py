...
def get_lep_selection_str():
    '''Do the lepton selection definition, the naming convention follows Dr Liu'''
    pdg_id_el = 11
    pdg_id_mu = 13
    ...
    #Tight
    lep1el_str = "abs(lep1_pdgid)==%.0f && lep1_p4.Pt()>%.5f && fabs(lep1_p4.Eta())<%.5f && lep1_MiniIso < %.2f"\
    %(pdg_id_el, sel_pt_el, sel_eta_el,sel_miniRelIso_el) \
    +"&& lep1_passMediumID && lep1_relIso*lep1_p4.Pt() < 5"
    
    lep1mu_str = "abs(lep1_pdgid)==%.0f && lep1_p4.Pt()>%.5f && fabs(lep1_p4.Eta())<%.5f && lep1_MiniIso < %.2f"\
    ...
    #Veto
    lep1el_str = "abs(lep1_pdgid) == %.0f && lep1_p4.Pt()>%.5f && abs(lep1_p4.Eta())<%.5f && lep1_MiniIso < %.2f"\
    %(pdg_id_el, veto_pt_el, veto_eta_el,veto_miniRelIso_el) +" && lep1_passVeto "
    lep1mu_str = "abs(lep1_pdgid) == %.0f && lep1_p4.Pt()>%.5f && abs(lep1_p4.Eta())<%.5f && lep1_MiniIso < %.2f"\
...
def get_cut_dict():
    ...
    cut_dict['passOneLep'] = "("+lep1_tight +"&& lep1_relIso*lep1_p4.Pt() < 5)"
    ...
    region_cut_dict["3jet_mct100"] = PSR_cut + "&& mct > 100"
    region_cut_dict["3jet_mct150"] = PSR_cut + "&& mct > 150"
