...
def draw_histo(file_name, var_name, xmin, xmax):
    ...
    if not("output" in file_name):
        xmin =  t.GetMinimum(var_name)
        if xmin<-900: 
            xmin = 0
        xmax =  t.GetMaximum(var_name)
    myhist = ROOT.TH1F("myhist"+rand_str,"myhist"+rand_str, 40, xmin, xmax);#,bin_num,xmin,xmax
...
if __name__ == "__main__": 
    names = get_all_branches("/home/users/siconglu/Mia_WH_Analysis/WHAnalysis/onelepbabymaker/output_sicheng_singlelep.root")
    #names = get_all_branches("../root_file_temp/Sicong_20180408/TChiWH_700_1_test.root")
    plot_folder_name = "WH_Comparing_Babies_Single/"
    processes = []
    num_cores = 20
    total_num = len(names)
    ...
    for index in range(len(names)):
        ...
        if (index+1)%num_cores == 0 :            
            ...
            print("Expect to complete in %.2f miniutes"%(1.*(current_time-start_time)/index*(total_num-index)/60.))
            processes = []
